"""
MLBB Duo Finder Bot — UPDATED VERSION
E'lon berganlarni ko'rsatish va tanlab xabar yuborish
...
# ─────────────────────────────────────────────
TOKEN = "8830561217:AAGfXR1HhpZMgvmeJW7DUaF2xdiby1j845c"
DB    = "mlbb.db"
# ─────────────────────────────────────────────
    ...
    try:
        async with aiosqlite.connect(DB) as db:
            ...
            for user_id, ann_rank, roles_str in rows:
                ...
                # Agar bir xil rol bo'lsa qo'shish
...
async def send_profile(target: types.Message, user_id: int):
    ...
    # Agar e'lon berilgan bo'lsa, o'chirish tugmasini qo'shish
    if announcement:
...
# ─────────────────────────────────────────────
#  SHERIK TOPISH (FAQAT E'LON BERGANLAR)
# ─────────────────────────────────────────────
...
async def cb_toggle_finding_role(call: types.CallbackQuery, state: FSMContext):
    ...
    if role == "done":
        ...
        # E'LON BERGAN SHERIKLAR QIDIRISH
        ...
        if not matched:
            await call.message.edit_text(
                ...
                f"💡 Foydalanuvchilar e'lon berishini kutib turing!",
                parse_mode="HTML"
            )
            await state.clear()
            return
        ...
        # SHERIKLAR KO'RSATISH - HAR BIRIGA XABAR TUGMASI
